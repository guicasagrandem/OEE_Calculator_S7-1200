...
4.  Unzip the downloaded `.zip` file to a folder of your choice on your computer.

### 3. Restore the Project in TIA Portal

1.  Open TIA Portal.
2.  In the top menu, go to **`Project` > `Retrieve`**.
3.  Navigate to the folder where you unzipped the project.
4.  Select the project backup file, which has the extension `.zap19` (the name should be `OEE_REAL_TIME.zap19`).
5.  Click `Open` and follow the instructions to restore the project.
...
| `AverageCycleTime` | `REAL` | Average cycle time in seconds for the **currently active shift**. | This is the average of the cycle times for all valid parts produced in the current shift. **May take a few cycles to update after shift reset,** as it needs at least 2 cycles to calculate the first valid time delta. |
...
### 5. Configuration of Retentive Variables

To ensure that OEE data is not lost after a PLC power cycle or restart, it is **crucial** that certain variables in the `FB_OEE_Calculator` instance Data Block are configured as **retentive (Retain)**.

1.  In TIA Portal, open the instance Data Block of your `FB_OEE_Calculator` (usually named `FB_OEE_Calculator_DB` or similar).
2.  In the variable table, for each variable listed below, check the box in the "Retain" column:
    * `TotalDowntime_S1`, `TotalDowntime_S2`, `TotalDowntime_S3`
    * `TotalCyclesCount_S1`, `TotalCyclesCount_S2`, `TotalCyclesCount_S3`
    * `ScrapPartsCount_S1`, `ScrapPartsCount_S2`, `ScrapPartsCount_S3`
    * `GoodPartsCount_S1`, `GoodPartsCount_S2`, `GoodPartsCount_S3`
    * `TotalCycleTimeSum_S1`, `TotalCycleTimeSum_S2`, `TotalCycleTimeSum_S3`
    * `CyclesForAverageCount_S1`, `CyclesForAverageCount_S2`, `CyclesForAverageCount_S3`
    * `LastCycleTimestamp_S1_ms`, `LastCycleTimestamp_S2_ms`, `LastCycleTimestamp_S3_ms`
    * `LastResetTimestamp_ms`
    * `Downtime_StartTime_Stamp_ms`
    * `ShiftActualStartTime_ms`

### 6. Download to the PLC

Compile the project and download it to your S7-1200 PLC.
...
* [FB_OEE_1.png](FB_OEE_1.png)
* [FB_OEE_2.png](FB_OEE_2.png)
